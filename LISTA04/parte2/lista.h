...
public:
    ...
    void remover(int idx) {
        ...
        // desloca para a esquerda para "tampar o buraco"
    ...
    virtual int buscar(T valor) {
        for (int i = 0; i < num_itens; i++) {
            if (itens[i] == valor) {
                return i + 1; // lista indexada em 1
            }
        }
        return -1;
    }
...
public:
    ...
    int buscar(T v) override {
        // Busca binária iterativa (array interno é 0..num_itens-1)
        int l = 0;
        int r = this->num_itens - 1;

        while (l <= r) {
            int mid = l + (r - l) / 2;

            if (this->itens[mid] == v) {
                return mid + 1; // indexada em 1
            }

            if (this->itens[mid] < v) {
                l = mid + 1;
            } else { // this->itens[mid] > v
                r = mid - 1;
            }
        }
